...
CACHE_MAX_SEG = 900 # 15 Minutos de los últimos datos

# Orden definido por usuario
ESTACIONES = [
    {"nombre": "Q-TAJ'",  "location_code": "Q_TAJ_prima", "lat": -9.5609,   "lng": -77.07220,   "buffer_m": 1000},
    # {"nombre": "Usupallares", "location_code": "USUPALLARES","lat": -9.55422, "lng": -77.07305, "buffer_m": 1000},
    {"nombre": "Q-2C",    "location_code": "2CRUCES",     "lat": -9.56023,  "lng": -77.05986,   "buffer_m": 1000},
    {"nombre": "Q-AN",    "location_code": "QUEBRADA",    "lat": -9.55501,  "lng": -77.08584,   "buffer_m": 1000},
...
# CACHÉ GUARDADO
# ══════════════════════════════════════════════════════════
def guardar_cache(nombre, obs, pron, max_val, max_time):
  try:
    cache = {}
    ...
    cache[nombre] = {
        "ts" : time.time(),
        "obs": [{"time": r["time"].isoformat(), "value": r["value"]} for r in obs],
        "pron": [{"time": r["time"].isoformat(), "value": r["value"]} for r in pron],
        "max_val": max_val,
        "max_time": max_time.isoformat() if max_time else None,
    }
    with open(CACHE_FILE, "w") as f:
      json.dump(cache, f)
    print(f" [{nombre}] Caché guardado")
  except Exception as e:
    print(f" [{nombre}] Error guardando caché: {e}")

# ══════════════════════════════════════════════════════════
# LEER CACHÉ GUARDADO
# ══════════════════════════════════════════════════════════
def leer_cache(nombre):
  try:
    ...
    if nombre not in cache:
      return None
    entry = cache[nombre]
    edad = time.time() - entry["ts"]
    if edad > CACHE_MAX_SEG:
      print(f"[{nombre}] Caché expirado ({edad/60:.1f} min) excluir")
      return None
    ultima = datetime.fromisoformat(entry["obs"][-1]["time"]) if entry ["obs"] else None
    hora_utlima = ultima.strftime("%H:%M") if ultima else "-"
    print(f" [nombre] Usando caché ({edad/60:.1f} min) . ultima_data: {hora_utlima}")
    obs = [{"time": datetime.fromisoformat(r["time"]), "value": r["value"]} for r in entry ["obs"]]
    pron = [{"time": datetime.fromisoformat(r["time"]), "value": r["value"]} for r in entry ["pron"]]
    max_time = datetime.fromisoformat(entry["max_time"]) if entry ["max_time"] else None
    return obs, pron, entry["max_val"], max_time, hora_utlima
  except Exception as e:
    print(f"[{nombre}] Error leyendo caché: {e}")
    return None
# ══════════════════════════════════════════════════════════
# COLOR
# ══════════════════════════════════════════════════════════
...
def generar_mapa(resultados):
    ...
    m = folium.Map(
        ...
        zoom_start=14.5, # Anterior 13, cambió el 11/05/2026
    ...
    for est in resultados:
        ...
        folium.CircleMarker(
            ...
            popup=folium.Popup(f"""
                ...
                Buffer: <b>{km:.0f} km</b><br>
                {'<b style = "color:#f59e0b"> Datos desde caché</b>' if est['desde_cache'] else ''}
                </div>""", max_width=220),
            tooltip=f"{est['nombre']} · {est['max_val']:.2f} μg/m³"
        ).add_to(m)

        # Label sobre el buffer
        folium.Marker(
            location=[est["lat"], est["lng"]],
            icon=folium.DivIcon(
                html=f"""<div style="
...
def preparar_chart_data(resultados):
    ...
    for est in resultados:
        ...
        if pron:
            inicio    = pron[0]["time"]
            ventana_i = inicio - timedelta(hours=2) # Posiblemente reducir a 1 hora
            ...
            eje_x = []
        # Extensión de la línea de observación a la de pronóstico
        # última corrección nowcasting
        # obs_continuo = (obs + [pron[0]]) if obs and pron else obs
        # Ajuste a visualizar solo una hora del observado
        obs_filtrada = [r for r in obs if r["time"] >= ventana_i]
        obs_continuo = (obs_filtrada + [pron[0]]) if obs_filtrada and pron else obs_filtrada
        obs_data  = [{"x": r["time"].strftime("%H:%M"), "y": r["value"]} for r in obs_continuo]
        ...
        charts.append({
            ...
            "desde_cache": est["desde_cache"],
            "hora_utlima": est["hora_ultima"],
        })
...
<header>
  <div class="logo">PROYECCIÓN CADA 5 MINUTOS - NOWCASTING PM10</div>
  <div class="hdr-right">
    <div>Corte: <span>{hora_corte}</span></div>
    <div>Actualizado: <span>{fecha_act}</span></div>
    <div>Próx. Act. <span>{hora_prox}</span></div>
  </div>
</header>
...
CHART_DATA.forEach((est, i) => {{
  ...
  if (est.obs.length) {{
    traces.push({{
      ...
      marker:{{color:'#1A1A1A', size:4, symbol:'circle'}},
  ...
  if (est.pron.length) {{
    traces.push({{
      ...
      type:'scatter', mode:'lines+markers',
      line:{{ color:'#0070A3', width:2 }},
      marker:{{color:'#0070A3', size:6, symbol:'circle'}},
      hovertemplate:'%{{x}}<br>%{{y:.2f}} μg/m³<extra></extra>',
    }});
  }}
...
# ══════════════════════════════════════════════════════════
# CONSULTAR ESTACIÓN — con caché
# ══════════════════════════════════════════════════════════
def consultar_estacion(est, token, ts_fijo):
    try:

        print(f"\n[{est['nombre']}] Consultando API...")
        print("=" * 55)
        ...
        print(f"  [{est['nombre']}] Obs:{len(obs)} Pron:{len(pron)} Máx:{max_val:.2f} {emoji} {cat}")

        guardar_cache(est["nombre"], obs, pron, max_val, max_time)
        ...
        return {
            ...
            "pronostico":  pron,
            "desde_cache": False,
            "hora_ultima": obs[-1]["time"].strftime("%H:%M") if obs else "—",
        }

    except Exception as e:
        print(f"  [{est['nombre']}] ❌ Error API: {e} → revisando caché...")
        cached = leer_cache(est["nombre"])

        if cached:
            obs, pron, max_val, max_time, hora_ultima = cached
            return {
                "nombre":      est["nombre"],
                "lat":         est["lat"],
                "lng":         est["lng"],
                ...
                "pronostico":  pron,
                "desde_cache": True,
                "hora_ultima": hora_ultima,
            }
        else:
            print(f"  [{est['nombre']}] Sin caché válido → excluida del dashboard")
            return None
...
if __name__ == "__main__":
    ...
    print("=" * 55)
    print("  PM10 Nowcasting · Antamina")
    print(f"  {ahora.strftime('%d/%m/%Y %H:%M:%S')} (Hora Perú)")
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        for futuro in as_completed(futuros):
            ...
            if resultado is not None:
                resultados_dict[est["nombre"]] = resultado

    # Orden original, excluyendo estaciones sin datos
    resultados = [
        resultados_dict[est["nombre"]]
        for est in ESTACIONES
        if est["nombre"] in resultados_dict
    ]

    if not resultados:
        print("❌ Sin datos para ninguna estación. Abortando.")
        exit(1)
    ...
    print(f"\n[✓] index.html generado · Corte: {ahora.strftime('%H:%M')} · Estaciones: {len(resultados)}/3")
